...
from implicitPBT_module.networks.soft_modular_model import SoftModularActorCriticNet
from implicitPBT_module.networks.simple_ensemble_model import SimpleEnsembleActorCriticNet
from implicitPBT_module.env_wrapper.latent_parameter_augmented_envwrapper \
    import LatentGaussianAugmentedPettiingZooEnvWrapper, LatentGaussianAugmentedMultiAgentEnvWrapper
from implicitPBT_module.env_wrapper.flatten_envwrapper import FlattenEnvWrapper
from implicitPBT_module.env_wrapper.onehot_wrapper import one_hot_obs_wrapper
from implicitPBT_module.networks.base_networks import MLPBase
...
class Args(object):
    ...
    def _post_init_processing(self) -> None:
        if self.game.__name__.startswith('gym_derk'):
            self.game_name = 'derk'
            self.game_type = 'gym_derk'
        else:
...
def is_adversary(agent_id: str) -> bool:
    return (('adversary' in agent_id)
            ...
            or (agent_id.startswith('marine') and ('3' in agent_id or '4' in agent_id or '5' in agent_id))
            or 'rear' in agent_id)

def get_config(args: Args, cmd_args=None):
    # num_rollouts = 2
    ...
    def env_creator(allow_derk_render=False):
        ...
        elif 'starcraft' in args.game_name:
            env = args.game.env(map_name="3m_multi_close")
        elif 'derk' in str(args.game_name):
            env = args.game.DerkEnv(turbo_mode=(not allow_derk_render))
        elif 'ant' in str(args.game_name):
            env = args.game.RoboschoolAntWrapper()
        elif 'cheetah' in str(args.game_name):
            env = args.game.RoboschoolCheetahWrapper()
        elif 'hopper' in str(args.game_name):
            env = args.game.RoboschoolHopperWrapper()
        else:
            env = args.game.env()
        ...
        use_ma_wrapper = 'ant' in str(args.game_name) or 'cheetah' in str(args.game_name) or 'hopper' in str(args.game_name)
        if use_ma_wrapper:
            env = LatentGaussianAugmentedMultiAgentEnvWrapper(env,
                                                              latent_parameter_dim=args.latent_para_dim,
                                                              gaussian_std=GAUSSIAN_STD,
                                                              use_dict_obs_space=args.use_dict_obs_space)
        else:
            env = LatentGaussianAugmentedPettiingZooEnvWrapper(env,
                                                               latent_parameter_dim=args.latent_para_dim,
                                                               gaussian_std=GAUSSIAN_STD,
                                                               use_dict_obs_space=args.use_dict_obs_space)
        return env
    # 3. Register env, and get trainer_class
    not_use_pettingzoo = 'ant' in str(args.game_name) or 'cheetah' in str(args.game_name) or 'hopper' in str(args.game_name)
    regis_env = env_creator() if not_use_pettingzoo else PettingZooEnv(env_creator())
    register_env(args.game_name,
                 lambda config: regis_env)
    trainer_class = get_agent_class(args.alg_name)

    # 4. Extract space dimensions
    test_env = env_creator()  if not_use_pettingzoo else PettingZooEnv(env_creator(allow_derk_render=True))
    ...
    try:
        ...
        # we have not specified horizon for starcraft
        print(e)
        pass
    # Default: False
    ...
    if args.train_setting == 'single_policy':
        ...
        main_config, test_config = get_simple_ensemble_training_config(config, ensemble_size=cmd_args.ensemble_size)
    else:
...
# def debug_test_env(env):
#     for iter in range(30):
#         env.reset()
#         action_space = env.action_space
#         done = False
#         while not done:
#             print(f'num of iter: {iter}')
#             obs, reward, done, info = env.step({agent: action_space.sample() for agent in ['front_legs', 'rear_legs']})
#             print(f'config 269: obs {obs}, reward {reward}, done {done}, info {info}')
#             done = done['__all__']
