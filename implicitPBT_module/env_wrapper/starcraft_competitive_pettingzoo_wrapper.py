from smac.env import StarCraft2Env, StarCraft2EnvMulti
...
class smac_parallel_env(ParallelEnv):
    def __init__(self, env, max_cycles):
        ...
        # compatible with competitive env
        if len(observation_size) == 2 and observation_size[0] == observation_size[1]:
    ...
    def _init_agents(self):
        ...
        self.agents_id = {}
        i = 0
        enemies_dict = {key + len(self.env.agents): self.env.enemies[key] for key in self.env.enemies.keys()}
        for agent_id, agent_info in {**self.env.agents, **enemies_dict}.items():
    ...
    def step(self, all_actions):
        action_list = [0] * (self.env.n_agents + self.env.n_enemies)
        self.agents = [
...
class _parallel_env(smac_parallel_env, EzPickle):
    ...
    def __init__(self, max_cycles, **smac_args):
        ...
        env = StarCraft2EnvMulti(**smac_args)
