...
class RoboschoolHopperWrapper(MultiAgentEnv):
    def __init__(self) -> None:
        ...
        new_action_space = gym.spaces.box.Box(low=original_action_space.low[:2],
                                              high=original_action_space.high[:2],
    ...
    def step(self, actions):
        ...
        action = np.concatenate([front_action[:1], np.array([(front_action[1] + rear_action[0]) / 2.0], dtype=np.float32), rear_action[-1:]])
