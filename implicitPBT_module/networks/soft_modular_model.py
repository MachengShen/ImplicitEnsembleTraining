...
from implicitPBT_module.networks.utils import basic_init, uniform_init, null_activation
from implicitPBT_module.utils.check_nan_inf import assert_nan_inf
...
class ModularGatedCascadeCondNet(nn.Module):
    def __init__(self, observation_space, action_space, num_outputs,
        ...
        if self._network_type == 'policy':
            if hasattr(action_space, 'n'):
                ...
                # 'Box space'
                # correspond to gaussian mean and logofstd
                output_shape = 2 * action_space.shape[0]
        else:
            output_shape = 1
        ...
        if self._use_dict_obs_space:
            ...
            if hasattr(observation_space.original_space.spaces['original_obs'], 'spaces'):
                # for starcraft2, compound space
                base_input_shape = (observation_space.original_space.spaces['original_obs'].spaces['action_mask'].shape[0] +
                                    observation_space.original_space.spaces['original_obs'].spaces['observation'].shape[0], )
            else:
                base_input_shape = observation_space.original_space.spaces['original_obs'].shape
            print(f'soft_modular, line 85 {base_input_shape}, {observation_space}')
        else:
    ...
    def forward(self, input_dict, state, seq_lens, return_weights = False, embedding_input=None):
        ...
        if self._use_dict_obs_space:
            ...
            if isinstance(x, collections.OrderedDict):
                x = torch.cat([x['action_mask'].float(), x['observation']], dim=-1)
        ...
        if self._use_latent_embedding:
            ...
            # for extracting grad of the gaussian noise
